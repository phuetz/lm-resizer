...
//!    Python `lm-resizer.cli.toin_publish` CLI emits.
...
use lm_resizer_core::transforms::recommendations::{
    AuthMode, RecommendationStore, RecommendationsError,
};

/// Minimal tempdir helper. Project convention is to avoid a
/// dev-dependency on `tempfile` (see `crates/lm-resizer-parity` for the
...
fn tempdir() -> TempDir {
    ...
    let p = std::env::temp_dir().join(format!(
        "lm-resizer-recommendations-{nanos}-{:?}",
