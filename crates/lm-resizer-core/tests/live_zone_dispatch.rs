...
use lm_resizer_core::transforms::live_zone::DEFAULT_MODEL;
...
#[test]
fn source_code_tool_result_routes_to_source_compressor() {
    let code = "
// module overview
fn main() {
    // input setup
...
"
    ...
    let out = dispatch(&body);
    let LiveZoneOutcome::Modified { manifest, new_body } = &out else {
        panic!("expected source code compression, got {out:?}");
    };
    assert!(!new_body.get().contains("// module overview"));
    ...
    match action {
        BlockAction::Compressed { strategy, .. } => {
            assert_eq!(strategy, "source_compressor");
        }
        other => panic!("expected source_compressor Compressed, got {other:?}"),
    }
}
