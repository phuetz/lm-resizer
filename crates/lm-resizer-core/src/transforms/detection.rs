...
mod tests {
    ...
    fn grep_search_results_route_to_plain_text_per_locked_design() {
        ...
        assert!(
            ...
            "grep output should route to SearchResults, PlainText, or SourceCode, got {result:?}"
        );
    }
    ...
    fn build_log_output_routes_via_chain() {
        ...
        assert!(
            matches!(
                got,
                ContentType::BuildOutput | ContentType::PlainText | ContentType::SourceCode
            ),
            "build log should route to BuildOutput, PlainText, or SourceCode, got {got:?}"
        );
    }
