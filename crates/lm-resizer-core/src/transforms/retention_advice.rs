...
pub struct RetentionRange {
    ...
    /// What the range is, in the producer's vocabulary (`Method`, `Class`,
    /// `Dependency`, ...). Protecting lines needs none of it. Eliding a body
    /// does: a namespace range covers the whole file, and only a producer that
    /// says "this one is a method" licenses touching what is inside.
    /// Absent means "unknown kind", which never licenses an elision.
    #[serde(default)]
...
pub struct RetentionAdvice {
    ...
    /// Language the producer parsed the bytes as (`csharp`, `rust`, ...).
    /// Diagnostic, and a hint for which comment syntax a marker may use.
    #[serde(default)]
    pub language: Option<String>,
    /// Contract marker, e.g. `lm-resizer.retention-advice/1`. Informational.
    #[serde(default)]
