...
use super::source_compressor::SourceCompressor;
...
pub enum BlockAction {
    ...
    /// Examples: `PlainText` (no compressor), `Html` (no compressor),
...
fn source_compressor() -> &'static SourceCompressor {
    static INSTANCE: OnceLock<SourceCompressor> = OnceLock::new();
    INSTANCE.get_or_init(SourceCompressor::default)
}
...
///    `lm_resizer_retrieve(hash="HASH")` to recover the original bytes.
///
...
///    `lm-resizer/ccr/tool_injection.py` keeps working — it matches
...
fn dispatch_compressor(text: &str, content_type: ContentType) -> DispatchResult {
    ...
    match content_type {
        ...
        ContentType::SourceCode => {
            let result = source_compressor().compress(text);
            if result.compressed == result.original {
                return DispatchResult::NoOp {
                    content_type: content_type.as_str(),
                };
            ...
            DispatchResult::Compressed {
                strategy: STRATEGY_SOURCE_COMPRESSOR,
                compressed: result.compressed,
            }
        }
...
/// `lm-resizer-core` is independent of the proxy crate.
...
pub fn compress_openai_responses_live_zone(
    ...
    let mut lm_resizer_retrieve_call_ids: HashSet<&str> = HashSet::new();
    for item in items {
        if item.get("type").and_then(Value::as_str) != Some("function_call") {
            continue;
        }
        let name = item.get("name").and_then(Value::as_str).unwrap_or("");
        if name == "lm_resizer_retrieve" || name.ends_with("__lm_resizer_retrieve") {
            if let Some(call_id) = item.get("call_id").and_then(Value::as_str) {
                lm_resizer_retrieve_call_ids.insert(call_id);
            }
        }
    }
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        match type_tag {
            "function_call_output" | "local_shell_call_output" | "apply_patch_call_output" => {
                ...
                if call_id.is_some_and(|id| lm_resizer_retrieve_call_ids.contains(id)) {
                    continue;
                }
...
mod openai_responses_tests {
    ...
    #[test]
    fn lm_resizer_retrieve_output_not_in_live_zone() {
            ...
            "input": [
                {
                    ...
                    "name": "mcp__lm_resizer__lm_resizer_retrieve",
                    "arguments": "{}"
                },
                {
