...
// Monotonic clock shim: real `std::time::Instant` on native, a zero-cost
// stand-in on wasm32 (where `Instant::now()` panics). On wasm the TTL never
// fires (elapsed is always 0); capacity eviction still applies.
