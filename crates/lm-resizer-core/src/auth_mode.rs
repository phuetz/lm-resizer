...
//!   `User-Agent`, never inject `X-lm-resizer-*`, never strip
//!   `accept-encoding`).
//!
...
/// Three auth-mode classes lm-resizer routes compression policy through.
///
...
pub enum AuthMode {
    ...
    /// `X-lm-resizer-*`; never mutate `User-Agent`.
    Subscription,
}
...
/// `crates/lm-resizer-core/benches/auth_mode.rs` asserts <10us / call.
...
mod inline_tests {
    ...
    //! lm-resizer-core --lib` exercises the helper without pulling in
    //! the integration-test binary. The exhaustive test matrix lives
    //! in `crates/lm-resizer-core/tests/auth_mode.rs`.
