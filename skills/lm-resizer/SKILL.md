...
argument-hint: "[command to wrap]"
allowed-tools: Bash(lm-resizer *), Bash(*/lm-resizer *), Read
---
...
`lm_resizer_compress`, `lm_resizer_tool_output` (post-execution reduction of output you already have; never runs commands), `lm_resizer_retrieve`, `lm_resizer_stats`. Install once per project: `lm-resizer install --client claude --scope project`.
